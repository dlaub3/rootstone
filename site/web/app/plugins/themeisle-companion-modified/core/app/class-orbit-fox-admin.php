...
class Orbit_Fox_Admin {
	...
	public function menu_pages() {
		add_management_page(
			__( 'Orbit Fox', 'themeisle-companion-modified' ), __( 'Orbit Fox Companion', 'themeisle-companion-modified' ), 'manage_options', 'obfx_companion',
			array(
				$this,
				'page_modules_render',
			)
	...
	public function try_module_save( $data ) {
		...
		$response['type'] = 'error';
		$response['message'] = __( 'No module found! No data was updated.', 'themeisle-companion-modified' );
		if ( isset( $modules[ $data['module-slug'] ] ) ) {
			$module = $modules[ $data['module-slug'] ];
			unset( $data['noance'] );
			unset( $data['module-slug'] );
			$response['type'] = 'warning';
			$response['message'] = __( 'Something went wrong, data might not be saved!', 'themeisle-companion-modified' );
			$result = $module->set_options( $data );
			if ( $result ) {
				$response['type'] = 'success';
				$response['message'] = __( 'Options updated, successfully!', 'themeisle-companion-modified' );
			}
		}
		return $response;
	}
	...
	public function obfx_update_module_options() {
		...
		$response['type'] = 'error';
		$response['message'] = __( 'Could not process the request!', 'themeisle-companion-modified' );
	...
	public function try_module_activate( $data ) {
		...
		$modules = $global_settings::$instance->module_objects;
		$response['type'] = 'error';
		$response['message'] = __( 'No module found!', 'themeisle-companion-modified' );
		if ( isset( $modules[ $data['name'] ] ) ) {
			$module = $modules[ $data['name'] ];
			$response['type'] = 'warning';
			$response['message'] = __( 'Something went wrong, can not change module status!', 'themeisle-companion-modified' );
			$result = $module->set_status( 'active', $data['checked'] );
			if ( $result ) {
				$response['type'] = 'success';
				$response['message'] = __( 'Module status changed!', 'themeisle-companion-modified' );
			}
		}
		return $response;
	}
	...
	public function page_modules_render() {
		...
		if ( $count_modules == 0 ) {
			...
			$empty_tpl = $rdh->get_partial(
				'empty',
				array(
					'title' => __( 'No modules found.', 'themeisle-companion-modified' ),
					'sub_title' => __( 'Please contact support for more help.', 'themeisle-companion-modified' ),
					'show_btn' => true,
				)
			);
			$panels = $rdh->get_partial(
				'empty',
				array(
					'title' => __( 'No active modules.', 'themeisle-companion-modified' ),
					'sub_title' => __( 'Activate a module using the toggles above.', 'themeisle-companion-modified' ),
					'show_btn' => false,
				)
			);
		}
